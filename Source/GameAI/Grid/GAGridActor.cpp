...
FVector2D AGAGridActor::TransformWorldToNormalizedGridSpace(const FVector& WorldPosition) const
{
	// First, transform the point into grid-local space
	// note, we drop the Z dimension at this point, by casting to a FVector2D
	FTransform GridTransform = GetActorTransform();
...
FVector2D AGAGridActor::GetCellNormalizedGridSpacePosition(const FCellRef& CellRef) const
{
...
FVector AGAGridActor::TransformNormalizedGridSpaceToWorld(const FVector2D& UniformGridSpacePosition) const
{
