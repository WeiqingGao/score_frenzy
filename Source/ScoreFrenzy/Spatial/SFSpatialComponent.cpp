...
USFSpatialComponent::USFSpatialComponent(const FObjectInitializer& ObjectInitializer)
...
ASFGridActor* USFSpatialComponent::GetGridActor() const
{
	ASFGridActor* Result = GridActorInternal.Get();
	if (Result)
	{
		return Result;
	}
...
USFPathComponent* USFSpatialComponent::GetPathComponent() const
{
	USFPathComponent* Result = PathComponentInternal.Get();
	if (Result)
	{
		return Result;
	}
	else
	{
		...
		if (Owner)
		{
			// Note, the USFPathComponent and the USFSpatialComponent are both on the controller
			Result = Owner->GetComponentByClass<USFPathComponent>();
			if (Result)
			{
...
APawn* USFSpatialComponent::GetOwnerPawn() const
{
	AActor* Owner = GetOwner();
	if (Owner)
	{
...
bool USFSpatialComponent::ChoosePosition(bool PathfindToPosition, bool Debug)
{
	bool Result = false;
	...
	ASFGridActor* Grid = GetGridActor();

	if (SpatialFunctionReference.Get() == NULL)
	{
		UE_LOG(LogTemp, Warning, TEXT("USFSpatialComponent has no SpatialFunctionReference assigned."));
		return false;
	}

	if (Grid == NULL)
	{
		UE_LOG(LogTemp, Warning, TEXT("USFSpatialComponent::ChoosePosition can't find a GridActor."));
		return false;
	}

	USFPathComponent* PathComponent = GetPathComponent();
	if (PathComponent == NULL)
	{
		UE_LOG(LogTemp, Warning, TEXT("USFSpatialComponent::ChoosePosition can't find a PathComponent."));
		return false;
	}
	...
	const USFSpatialFunction* SpatialFunction = SpatialFunctionReference->GetDefaultObject<USFSpatialFunction>();
	...
	FIntRect CellRect;
	FVector2D PawnGridSpaceLocation = Grid->TransformWorldToNormalizedGridSpace(OwnerPawn->GetActorLocation());
	// TransformWorldToNormalizedGridSpace returns normalized grid space (where 1 unit = 1 cell)
	// We need to convert to actual grid space (where 1 unit = CellScale)
	PawnGridSpaceLocation *= Grid->CellScale;
	
	Box += PawnGridSpaceLocation;
	...
	if (Grid->GridSpaceBoundsToRect2D(Box, CellRect))
	{
		...
		FSFGridBox GridBox(CellRect);

		// This is the grid map I'm going to fill with values
		FSFGridMap ScoreMap(Grid, GridBox, 0.0f);

		// Fill in this distance map using Dijkstra!
		FSFGridMap DistanceMap(Grid, GridBox, FLT_MAX);
		...
		if (!bDijkstraSuccess)
		{
			UE_LOG(LogTemp, Warning, TEXT("USFSpatialComponent::ChoosePosition - Dijkstra failed."));
			return false;
		}
		...
		for (const FSFFunctionLayer& Layer : SpatialFunction->Layers)
		{
		...
		FSFCellRef BestCell = FSFCellRef::Invalid;
		...
		for (int32 Y = ScoreMap.GridBounds.MinY; Y <= ScoreMap.GridBounds.MaxY; Y++)
		{
			for (int32 X = ScoreMap.GridBounds.MinX; X <= ScoreMap.GridBounds.MaxX; X++)
			{
				...
				if (!EnumHasAllFlags(Grid->GetCellData(CellRef), ESFCellData::CellDataTraversable))
				{
					continue;
				}
		...
		{
			UE_LOG(LogTemp, Warning, TEXT("USFSpatialComponent::ChoosePosition - No valid cell found."));
			return false;
		}
		...
		if (PathfindToPosition && BestCell.IsValid())
		{
			...
			if (!bPathSuccess)
			{
				UE_LOG(LogTemp, Warning, TEXT("USFSpatialComponent::ChoosePosition - BuildPathFromDistanceMap failed."));
...
void USFSpatialComponent::EvaluateLayer(const FSFFunctionLayer& Layer, const FSFGridMap& DistanceMap, FSFGridMap& ScoreMap) const
{
	APawn* OwnerPawn = GetOwnerPawn();
	...
	AActor* Owner = GetOwner();
	USFPerceptionComponent* PerceptionComponent = Owner->GetComponentByClass<USFPerceptionComponent>();
	
	// // Early out if we need player but don't have one
	// if ((Layer.Input == SI_TargetRange || Layer.Input == SI_LOS) && PlayerPawn == NULL)
	// {
	// 	UE_LOG(LogTemp, Warning, TEXT("USFSpatialComponent::EvaluateLayer - No player pawn found for target-based input."));
	// 	return;
	// }
	...
	if (Layer.Input == SI_TargetRange || Layer.Input == SI_LOS)
	{
		...
		FSFTargetState TargetState;
		FSFTargetView TargetView;
