...
#include "SFGridMap.generated.h"


// This allows us to define a set of floating point values over the grid defined by a ASFGridActor.
...
struct FSFGridBox
{
	...
	FSFGridBox() : MinX(INDEX_NONE), MaxX(INDEX_NONE), MinY(INDEX_NONE), MaxY(INDEX_NONE) {}
	FSFGridBox(int32 MinXIn, int32 MaxXIn, int32 MinYIn, int32 MaxYIn) : MinX(MinXIn), MaxX(MaxXIn), MinY(MinYIn), MaxY(MaxYIn) {}
	FSFGridBox(const FIntRect& Rect)
	{
		MinX = Rect.Min.X;
		MaxX = Rect.Max.X;
		MinY = Rect.Min.Y;
	...
	bool IsValidCell(const FSFCellRef& Cell) const;
};
...
struct FSFGridMap
{
	...
	FSFGridMap();
	FSFGridMap(int32 XCountIn, int32 YCountIn, float InitialValue);
	FSFGridMap(const ASFGridActor *Grid, float InitialValue);
	FSFGridMap(const ASFGridActor* Grid, const FSFGridBox &GridBoxIn, float InitialValue);
	...
	bool CellRefToLocal(const FSFCellRef& Cell, int32& X, int32& Y) const;
	...
	bool LocalToCellRef(int32 X, int32 Y, FSFCellRef& Cell) const;

	// Get the value that this map stores for the given grid cell
	bool GetValue(const FSFCellRef& Cell, float& ValueOut) const;
	...
	bool SetValue(const FSFCellRef& Cell, float Value);
