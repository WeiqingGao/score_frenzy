...
FSFCellRef FSFCellRef::Invalid(INDEX_NONE, INDEX_NONE);


ASFGridActor::ASFGridActor(const FObjectInitializer& ObjectInitializer)
...
#if WITH_EDITORONLY_DATA
void ASFGridActor::PostEditChangeProperty(FPropertyChangedEvent& PropertyChangedEvent)
{
...
void ASFGridActor::RefreshBoxComponent()
{
...
void ASFGridActor::RefreshDerivedValues()
{
	// Refresh HalfExtents
...
// Otherwise, if the point is outside the grid, it will return FSFCellRef::Invalid

FSFCellRef ASFGridActor::GetCellRef(const FVector& Point, bool bClamp) const
{
...
FVector ASFGridActor::GetCellPosition(const FSFCellRef& CellRef) const
{
...
bool ASFGridActor::IsCellRefInBounds(const FSFCellRef& CellRef) const
{
	return (CellRef.X >= 0) && (CellRef.X < XCount) && (CellRef.Y >= 0) && (CellRef.Y < YCount);
}

FVector2D ASFGridActor::GetCellGridSpacePosition(const FSFCellRef& CellRef) const
{
	float HalfScale = 0.5f * CellScale;
...
// Transform a world-space position into normalized grid space
// Normalized grid space is the space in which a cell is 1.0 units wide
FVector2D ASFGridActor::TransformWorldToNormalizedGridSpace(const FVector& WorldPosition) const
{
	// First, transform the point into grid-local space
	// note, we drop the Z dimension at this point, by casting to a FVector2D
	FTransform GridTransform = GetActorTransform();
	FVector2D LocalPoint = FVector2D(GridTransform.InverseTransformPosition(WorldPosition));

	LocalPoint += HalfExtents;		// Now LocalPoint is relative the to the (0, 0) corner of grid

	return FVector2D(LocalPoint.X / CellScale, LocalPoint.Y / CellScale);
}

// Get the normalized grid-space position of the center of the given cell
// Normalized grid space is the space in which a cell is 1.0 units wide
// Therefore, the normalized grid-space position of cell (0, 0), for example, is (0.5, 0.5)
FVector2D ASFGridActor::GetCellNormalizedGridSpacePosition(const FSFCellRef& CellRef) const
{
	return FVector2D(float(CellRef.X) + 0.5f, float(CellRef.Y) + 0.5f);
}


// Transform a normalized grid-space position into world space
FVector ASFGridActor::TransformNormalizedGridSpaceToWorld(const FVector2D& UniformGridSpacePosition) const
{
	FVector2D GridSpacePoint = UniformGridSpacePosition * CellScale;

	GridSpacePoint -= HalfExtents;

	// First, transform the point into grid-local space
	// note, we drop the Z dimension at this point, by casting to a FVector2D
	FTransform GridTransform = GetActorTransform();
	FVector LocalPoint3D(GridSpacePoint, 0.0f);

	return GridTransform.TransformPosition(LocalPoint3D);
}


ESFCellData ASFGridActor::GetCellData(const FSFCellRef &CellRef) const
{
...
float ASFGridActor::GetCellHeightData(const FSFCellRef& CellRef) const
{
	int32 CellIndex = CellRefToIndex(CellRef);
...
bool ASFGridActor::GridSpaceBoundsToRect2D(const FBox2D& Box, FIntRect &RectOut) const
{
	float HalfScale = 0.5f * CellScale;
...
bool ASFGridActor::RefreshDataFromNav()
{
	bool Result = false;
									...
									for (int Y = GridBox.Min.Y; Y <= GridBox.Max.Y; Y++)
									{
										for (int X = GridBox.Min.X; X <= GridBox.Max.X; X++)
										{
											...
											if (!IsOutside)
											{
												...
												if (!EnumHasAnyFlags(CellData[CellIndex], ESFCellData::CellDataTraversable))
												{
													EnumAddFlags(CellData[CellIndex], ESFCellData::CellDataTraversable);
													bFirst = true;
												}
...
bool ASFGridActor::RefreshDebugMesh()
{
	...
	// TODO: do a raycast to to figure out where the ground actually is
	{
		...
		for (int32 Y = 0; Y <= YCount; Y++)				// note, the use of the <= means we will have ONE MORE row and column or verts than cells
		{
			for (int32 X = 0; X <= XCount; X++)
			{
				...
				FSFCellRef BottomRight(X, Y);
				FSFCellRef TopRight(X, Y - 1);
				FSFCellRef TopLeft(X - 1, Y - 1);
				FSFCellRef BottomLeft(X - 1, Y);

				if (IsCellRefInBounds(BottomRight) && EnumHasAllFlags(GetCellData(BottomRight), ESFCellData::CellDataTraversable))
				{
				...
				if (IsCellRefInBounds(TopRight) && EnumHasAllFlags(GetCellData(TopRight), ESFCellData::CellDataTraversable))
				{
				...
				if (IsCellRefInBounds(TopLeft) && EnumHasAllFlags(GetCellData(TopLeft), ESFCellData::CellDataTraversable))
				{
				...
				if (IsCellRefInBounds(BottomLeft) && EnumHasAllFlags(GetCellData(BottomLeft), ESFCellData::CellDataTraversable))
				{
	...
	// Generate the triangles array with counter-clockwise winding
	{
		...
		for (int32 Y = 0; Y < YCount; Y++)
		{
			for (int32 X = 0; X < XCount; X++)
			{
				...
				if (EnumHasAllFlags(GetCellData(FSFCellRef(X, Y)), ESFCellData::CellDataTraversable))
				{
...
bool ASFGridActor::RefreshDebugTexture()
{
	bool Result = false;
	...
	/*
	{
		FSFGridBox Box(XCount / 4, XCount - XCount / 4, YCount / 4, YCount - YCount / 4);
		DebugGridMap = FSFGridMap(this, Box, 0.0f);
		for (int32 Y = Box.MinY; Y <= Box.MaxY; Y++)
		{
			for (int32 X = Box.MinX; X <= Box.MaxX; X++)
			{
				DebugGridMap.SetValue(FSFCellRef(X, Y), float((X-Box.MinX) + (Y - Box.MinY)));
			}
		}
	}
	*/
	...
	if (DebugMeshComponent)
	{
		...
		if (DebugGridMap.IsValid())
		{
			...
			for (int32 Y = 0; Y < YCount; Y++)
			{
				...
				{
					FSFCellRef CellRef(X, Y);
					ESFCellData CellData = GetCellData(CellRef);
					bool Traversable = EnumHasAllFlags(CellData, ESFCellData::CellDataTraversable);
		...
		else
		{
			for (int32 Y = 0; Y < YCount; Y++)
			{
				for (int32 X = 0; X < XCount; X++)
				{
					ESFCellData CellData = GetCellData(FSFCellRef(X, Y));
					bool Traversable = EnumHasAllFlags(CellData, ESFCellData::CellDataTraversable);
