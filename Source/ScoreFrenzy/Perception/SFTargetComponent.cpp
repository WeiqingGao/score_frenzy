...
USFTargetComponent::USFTargetComponent(const FObjectInitializer& ObjectInitializer)
...
ASFGridActor* USFTargetComponent::GetGridActor() const
{
	ASFGridActor* Result = GridActor.Get();
	if (Result)
	{
		return Result;
	}
...
void USFTargetComponent::OnRegister()
{
	Super::OnRegister();
	...
	if (Grid)
	{
		OccupancyMap = FSFGridMap(Grid, 0.0f);
	}
}

void USFTargetComponent::OnUnregister()
{
	Super::OnUnregister();
...
void USFTargetComponent::TickComponent(float DeltaTime, enum ELevelTick TickType, FActorComponentTickFunction* ThisTickFunction)
{
	...
	if (PerceptionSystem)
	{
		TArray<TObjectPtr<USFPerceptionComponent>> &PerceptionComponents = PerceptionSystem->GetAllPerceptionComponents();
		for (USFPerceptionComponent* PerceptionComponent : PerceptionComponents)
		{
			const FSFTargetView* TargetView = PerceptionComponent->GetTargetView(TargetGuid);
	...
	if (isImmediate)
	{
		...
		LastKnownState.State = SFTS_Immediate;
	...
	{
		LastKnownState.State = SFTS_Hidden;
	}

	if (LastKnownState.State == SFTS_Hidden)
	{
		OccupancyMapUpdate();
	}
...
void USFTargetComponent::OccupancyMapSetPosition(const FVector& Position)
{
	// TODO PART 4
	...
	if (Grid)
	{
		...
		FSFCellRef TargetPositionInCellRef = Grid->GetCellRef(Position, true);
...
void USFTargetComponent::OccupancyMapUpdate()
{
	const ASFGridActor* Grid = GetGridActor();
	if (Grid)
	{
		FSFGridMap VisibilityMap(Grid, 0.0f);
		...
		if (PerceptionSystem)
		{
			...
			for (USFPerceptionComponent* PerceptionComponent : PerceptionComponents)
			{
				...
				// Suggestion: you might find it useful to add a USFPerceptionComponent::TestVisibility method to the perception component.
				...
				const FSFCellRef Center = Grid->GetCellRef(RaycastStartLocation, /*bClamp=*/true);
		...
		FSFCellRef MaxProbabilityPosition;
		for (int32 y = MinY; y <= MaxY; ++y)
		{
			for (int32 x = MinX; x <= MaxX; ++x)
			{
				...
				if (!EnumHasAnyFlags(Grid->GetCellData(Cell), ESFCellData::CellDataTraversable)) continue;
				float P;
...
void USFTargetComponent::OccupancyMapDiffuse()
{
	// TODO PART 4
	// Diffuse the probability in the OMAP
	const ASFGridActor* Grid = GetGridActor();
	FSFGridMap Buffer = FSFGridMap(Grid, 0.0f);
		...
		for (int32 x = MinX; x <= MaxX; ++x)
		{
			...
			for (int32 CurrY = y - 1; CurrY <= y + 1; ++CurrY)
			{
				for (int32 CurrX = x - 1; CurrX <= x + 1; ++CurrX)
				{
					...
					FSFCellRef Neighbor(CurrX, CurrY);
					if (!Grid->IsCellRefInBounds(Neighbor)) continue;
					if (!EnumHasAnyFlags(Grid->GetCellData(Neighbor), ESFCellData::CellDataTraversable)) continue;
